...
class TestCase(_TestCaseBase, metaclass=PallasTestMetaclass):
  ...
  def setUp(self):
    ...
    # TODO(b/512782016): Re-enable once the bug is fixed.
    if (self.LOWERING_SEMANTICS == plgpu.LoweringSemantics.Warpgroup and
        is_multiprocess() and not is_nvshmem_used()):
      self.skipTest("wg semantics not supported in multiprocess mode")
...
class PallasCallMultimemTest(TestCase):
  ...
  def test_multimem_store_tma(self):
    if jax.process_index() > 2:
      return  # Only 2 processes needed.
    # TODO(b/512396897): Enable this test in multiprocess mode.
    if is_multiprocess() and not is_nvshmem_used():
